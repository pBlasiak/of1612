...
:
	...
	densityWeighted_(transportPropertiesDict_.lookup("densityWeighted")),
...
{
    calculateK();
	Info<< "rho1_ in smoothInterfaceProperties: " << rho1_ << endl;
	Info<< "rho2_ in smoothInterfaceProperties: " << rho2_ << endl;
	Info<< "densityWeighted_ in smoothInterfaceProperties: " << densityWeighted_ << endl;
}
...
Foam::smoothInterfaceProperties::surfaceTensionForce() const
{
	if (densityWeighted_)
	{
		Info<< "densityWeighted ..." << endl;
...
Foam::smoothInterfaceProperties::nearInterface() const
{
    return pos(alpha1_ - 0.01)*pos(0.99 - alpha1_);
}


bool Foam::smoothInterfaceProperties::read()
{
    alpha1_.mesh().solverDict(alpha1_.name()).lookup("cAlpha") >> cAlpha_;
    transportPropertiesDict_.lookup("sigma") >> sigma_;
	transportPropertiesDict_.subDict
		(wordList(transportPropertiesDict_.lookup("phases"))[0]).lookup("rho") >> rho1_;
	transportPropertiesDict_.subDict
		(wordList(transportPropertiesDict_.lookup("phases"))[1]).lookup("rho") >> rho2_;
	transportPropertiesDict_.lookup("densityWeighted") >> densityWeighted_;
